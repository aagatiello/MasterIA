#Regresi?n. Estimaci?n del modelo y gr?fico de dispersi?n
...
ks.test(err,"pnorm")#normalidad 

#predicci?n
mod1=lm(imdb ~ filmaffinity)
nuevo <- data.frame(filmaffinity=c(8,9))
...
datos <- data.frame(redes/10, radio, periodico, ventas)

modelo <- lm(formula = ventas ~ redes + radio + periodico, data = datos)
summary(modelo)
